...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "nginx" do |nginx|
    nginx.vm.synced_folder "C:\\Users\\Dmitry_Dubovtsev\\vagrant", "/vagrant_data", type: 'nfs'
    nginx.vm.box = "centos/7"
    nginx.vm.hostname = 'nginx.vg'
    ...
    nginx.vm.provision "shell", inline: <<-SHELL
        ...
        sudo cp /vagrant_data/share/tomcat.conf /etc/nginx/conf.d/
  ...
  config.vm.define "tomcat" do |tomcat|
    ...
    tomcat.vm.hostname = 'tomcat.vg'
    tomcat.vm.network "public_network", type: "dhcp"
